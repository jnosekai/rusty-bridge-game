...
SHADOW_OFFSET_Y = 14
SHADOW_OPACITY = 0.25
SHADOW_BLUR_SOURCE_PIXELS = 12
SHADOW_PADDING_SOURCE_PIXELS = 36
...
def build_foreground() -> tuple[Image.Image, tuple[int, int, int, int]]:
    ...
    foreground = clean_transparent_rgb(source.crop(alpha_bbox))
...
def render_layers(master: Image.Image, foreground: Image.Image, data: dict) -> Image.Image:
    ...
    fg = foreground.resize((BRIDGE_WIDTH, BRIDGE_HEIGHT), Image.Resampling.LANCZOS)
...
def main() -> None:
    ...
    data = placement_data(foreground, shadow, source_bbox)
